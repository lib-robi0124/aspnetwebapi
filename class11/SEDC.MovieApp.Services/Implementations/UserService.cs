...
namespace SEDC.MovieApp.Services.Implementations
{
    public class UserService : IUserService
    {
        ...
        public void DeleteUser(int id)
        {
            ...
            if (userDb == null)
            {
                throw new UserDataException($"User with {id} not found.");
            }
        ...
        public UserDto GetUserById(int id)
        {
            ...
            if (userDb == null)
            {
                throw new UserDataException($"User with id {id} not found.");
            }
        ...
        public void UpdateUser(UpdateUserDto updateUserDto)
        {
            ...
            if (userDb == null)
            {
                throw new UserDataException($"User with id {updateUserDto.Id} not found.");
            }
            //validations
            if (string.IsNullOrEmpty(updateUserDto.Username) || updateUserDto.Username.Length > 30)
            {
                throw new UserDataException("Username is required and should be less than 30 characters.");
            }
            if (!string.IsNullOrEmpty(updateUserDto.FirstName) && updateUserDto.FirstName.Length > 50)
            {
                throw new UserDataException("First name should be less than 50 characters.");
            }
            if (!string.IsNullOrEmpty(updateUserDto.LastName) && updateUserDto.LastName.Length > 50)
            {
                throw new UserDataException("Last name should be less than 50 characters.");
            }
            var userWithSameUsername = _userRepository.GetUserByUsername(updateUserDto.Username);
